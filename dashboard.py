...
.gaincard { background: #141821; border: 1px solid #1F2430; border-radius: 10px; padding: 14px 16px; margin-bottom: 10px; }
.gaincard .name { font-size: 12px; color: #6B7280; font-weight: 600; letter-spacing: 0.5px; }
.gaincard .val { font-size: 24px; font-weight: 700; color: #F0F2F6; font-family: 'JetBrains Mono', monospace; margin-top: 2px; }
.gaincard .delta { font-size: 12px; font-weight: 600; margin-top: 2px; }
.gaincard .delta.up { color: #4DD8C0; } .gaincard .delta.flat { color: #6B7280; }
.fixedtag { font-size: 11px; color: #4B5563; margin-top: 10px; }
...
with col1:
    ...
    def gaincard(name, val, fixed, fmt):
        d = val - fixed
        cls = "flat" if abs(d) < 1e-9 else "up"
        if abs(d) > 1e-12:
            arrow = "▲" if d > 0 else "▼"
            dtxt = f"{arrow} {abs(d):{fmt}} vs fixed {fixed:{fmt}}"
        else:
            dtxt = "= fixed"
        return (f'<div class="gaincard"><div class="name">{name}</div>'
                f'<div class="val">{val:{fmt}}</div>'
                f'<div class="delta {cls}">{dtxt}</div></div>')

    st.markdown('<div class="lbl" style="margin-top:18px">NN-Predicted Gains</div>', unsafe_allow_html=True)
    st.markdown(gaincard("Kp_v  ·  proportional", g_nn['Kp_v'], g_fix['Kp_v'], ".3f"), unsafe_allow_html=True)
    st.markdown(gaincard("Ki_v  ·  integral", g_nn['Ki_v'], g_fix['Ki_v'], ".2f"), unsafe_allow_html=True)
    st.markdown(f'<div class="gaincard"><div class="name">K_vin  ·  feedforward</div>'
                f'<div class="val">{g_nn["K_vin"]:.2e}</div></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="fixedtag">ω bounds held fixed: w_min={g_nn["w_min"]:.1f} · w_max={g_nn["w_max"]:.1f}</div>',
                unsafe_allow_html=True)
...
with col2:
    ...
    ylo = np.floor(ymin - pad); yhi = np.ceil(ymax + pad)
    ...
    fig.update_layout(
        height=440, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#0F1219",
